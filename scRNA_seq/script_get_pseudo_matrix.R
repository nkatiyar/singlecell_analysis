...
#        sig_res <- dplyr::filter(res_tbl, padj < padj_cutoff) %>%
 #               dplyr::arrange(padj)
	...
	breast_cancer_select_genes <- c("AHR", "ANXA1", "BMPR2", "CCND1", "CDK4", "CDK6", "CTNNB1", "E2F1", "EGFR", "FZD1", "GSK3B", "HIPK2", "ODC1", "PIGR", "PLK3", "PPP4R3A", "SMARCA4", "RAF1", "SKP1", "SP1", "STAT1", "TP53", "TCF7L1", "WNT4")	

	sig_norm_sub <- sig_norm[sig_norm$gene %in% breast_cancer_select_genes,]
	...
	breast_cancer_convert_mouse_genes <- convertHumanGeneList(breast_cancer_select_genes)

	test_gene <- c("Fzd1")
	sig_norm_subset <- sig_norm[sig_norm$gene %in% test_gene,]

	filename_heatmap_select_row <- paste0("../Heatmaps/Heatmap_", clusters[x], "_pseudobulk_DE_breast_cancer_genes_cluster_rows.pdf")
	pdf(filename_heatmap_select_row, height=30)
        pheatmap(sig_norm_subset[,2:length(colnames(sig_norm))], scale="row", cluster_rows=TRUE, show_rownames=TRUE, cluster_cols=FALSE, annotation_col=df)
        dev.off()
	
	filename_heatmap_select_column <- paste0("../Heatmaps/Heatmap_", clusters[x], "_pseudobulk_DE_breast_genes_cluster_cols.pdf")
	pdf(filename_heatmap_select_column, height=30)
        pheatmap(sig_norm[,2:length(colnames(sig_norm))], scale="row", cluster_rows=TRUE, show_rownames=TRUE, cluster_cols=TRUE, annotation_col=df)
        dev.off()
	
	filename_heatmap_select_rows_column <- paste0("../Heatmaps/Heatmap_", clusters[x], "_pseudobulk_DE_breast_genes_cluster_rows_and_cols.pdf")
	pdf(filename_heatmap_select_rows_column, height=30)
        pheatmap(sig_norm[,2:length(colnames(sig_norm))], scale="row", cluster_rows=TRUE, show_rownames=TRUE, cluster_cols=TRUE, annotation_col=df)
        dev.off()
